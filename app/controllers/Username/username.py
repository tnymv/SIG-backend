...
from app.schemas.username.username import UsernameBase, UsernameResponse, UsernameCreate
...
async def get_usernames(page: int = 1, limit: int = 5, db: Session = Depends(get_db)):
    try: 
        offset = (page - 1) * limit
        if page < 1 or limit < 1:
            raise HTTPException(status_code = 400, detail="La página y el límite deben ser mayores que 0")
        usernames = db.query(Username).offset(offset).limit(limit).all()
        return success_response([UsernameResponse.model_validate(user).model_dump(mode="json") for user in usernames])
...
async def create_username(username_data: UsernameBase, db: Session = Depends(get_db)):
    ...
    try:
        new_username = Username(
            ...
            password_hash=username_data.password_hash,
